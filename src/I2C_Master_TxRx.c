...
int main(void)
{
	...
	while(1) {
		...
		I2C_MasterSendData(I2C1, (uint8_t*)&RequestLength, 1, slaveAddress, I2C_REPEATSTART);
		//receive the length information
		I2C_MasterReceiveData(I2C1, &len, 1, slaveAddress, I2C_REPEATSTART);
		//send command to request data
		I2C_MasterSendData(I2C1, (uint8_t*)&RequestData, 1, slaveAddress, I2C_REPEATSTART);
		if(len > 1) {
			//receive the data
			I2C_MasterReceiveData(I2C1, (uint8_t*)&SampleData, len, slaveAddress, I2C_STOP);
		}
	}
